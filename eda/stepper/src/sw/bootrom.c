...
static volatile uint32_t* const REG_GPIO_STEPPER_COUNTER = (volatile uint32_t*)0xA0000020;
static volatile uint32_t* const REG_GPIO_STEPPER_CWCCW   = (volatile uint32_t*)0xA0000030;
...
void __attribute__((noreturn)) main(void)
{
    uint32_t counter = 0;
    uint32_t led_out = 1;
    const size_t LED_BITS = 6;
    const uint32_t stepper_counter_max = 27000000/100;
    const uint32_t stepper_counter_min = 27000000/1000;

    uint32_t stepper_counter = stepper_counter_max;
    bool stepper_cwccw = false;
    ...
    for(;;) {
        ...
        if( (counter & 0xf) == 0 ) {
            ...
            if( switch_input & 0x02 ) {
                stepper_counter -= 1;
            }
            if( switch_input & 0x04 ) {
                stepper_cwccw = true;
            }
            if( switch_input & 0x08 ) {
                stepper_cwccw = false;
            }
        }
        if( stepper_counter < stepper_counter_min ) {
            stepper_counter = stepper_counter_min;
        }
        if( stepper_counter > stepper_counter_max ) {
            stepper_counter = stepper_counter_max;
        }
        *REG_GPIO_STEPPER_COUNTER = stepper_counter;
        *REG_GPIO_STEPPER_CWCCW = stepper_cwccw;

        put_led_hex(stepper_counter | (stepper_cwccw ? 0x800000 : 0));
